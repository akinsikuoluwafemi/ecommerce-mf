...
export class CartController {
  ...
  async create(@Request() req, @Body() { id }: { id: string }): Promise<Cart> {
    ...
    const cart = this.carts[req.user.userId]; // Get the cart for the user
    const cartItem = cart.cartItems.find((cartItem) => cartItem.id === parseInt(id)) // Find the item in the cart

     if (cartItem) {
       cartItem.quantity += 1; // If the item is already in the cart, increase the quantity
     } else {
